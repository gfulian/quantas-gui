"""Dash callbacks for the executable Seismic workflow."""
...
from quantas_gui.workflows.seismic.presentation import (
    active_job_view,
    activity_view,
    diagnostic_text,
    failed_view,
...
from quantas_gui.workflows.seismic.schema import (
    build_request,
    format_stiffness,
    input_mode,
    seismic_form,
...
def register_seismic_callbacks(
    app: dash.Dash,
    service: SeismicWorkflowService,
) -> None:
...
def _register_input_callbacks(app: dash.Dash, service: SeismicWorkflowService) -> None:
    @app.callback(
        *_RESET_OUTPUTS,
        Output(SeismicIds.SOURCE, "data"),
        Output(SeismicIds.IMPORT_STATUS, "children"),
        Input(
        ...
        State(SeismicIds.SOURCE, "data"),
        State(SeismicIds.SESSION, "data"),
    ...
    def import_or_reset(
        ...
        try:
            ...
            _LOGGER.warning("Seismic source import rejected: %s", public_error_message(exc))
            return (
        ...
        updates[field_index["density"]] = imported.density
        source_payload = {
        ...
        return (
            *updates,
            ...
            message_banner(
                ...
                message=(
                    "The editable stiffness matrix and density were populated through the "
                    "public Quantas SEISMIC API."
                ),
                level="result",
            ),
        )
...
def _register_orchestration_callback(
    ...
    @app.callback(
        Output(SeismicIds.SESSION, "data"),
        Output(SeismicIds.SOURCE, "data", allow_duplicate=True),
        Output(SeismicIds.EVENTS, "data"),
        Output(SeismicIds.FORM_HOST, "className"),
        Output(SeismicIds.RUNTIME, "className"),
        Output(SeismicIds.RUNTIME, "children"),
        Output(SeismicIds.ACTIVITY, "className"),
        Output(SeismicIds.SUMMARY, "className"),
        Output(SeismicIds.SUMMARY, "children"),
        Output(SeismicIds.POLL, "disabled"),
        ...
        *_ERROR_OUTPUTS,
        ...
        Input(SeismicIds.POLL, "n_intervals"),
        Input(SeismicIds.CANCEL, "n_clicks", allow_optional=True),
        Input(SeismicIds.BACK, "n_clicks", allow_optional=True),
        Input(SeismicIds.OPEN_RESULTS, "n_clicks", allow_optional=True),
        State(SeismicIds.SESSION, "data"),
        State(SeismicIds.SOURCE, "data"),
        State(SeismicIds.EVENTS, "data"),
        *_VALUE_STATES,
        prevent_initial_call=True,
    )
    def orchestrate(
        ...
        *raw_values: Any,
        ...
        if triggered == SeismicIds.OPEN_RESULTS:
        ...
        if triggered == FormIds.submit(_SCHEMA.key):
            ...
            try:
                ...
                _LOGGER.exception("Unable to submit the SEISMIC job")
                return (
                    no_update,
                    no_update,
                    no_update,
        ...
        try:
            ...
            if triggered == SeismicIds.CANCEL:
            ...
            _LOGGER.exception("Unable to poll the SEISMIC job")
            status = JobStatus(
                state=JobState.FAILED,
        ...
        if status.state is JobState.SUCCEEDED:
            try:
                active = service.active_result(
                    handle,
                    jobname=str(workflow_session.get("jobname", "SEISMIC")),
                )
                tables = service.results.build_tables(active.reference)
            except Exception as exc:
                _LOGGER.exception("Unable to prepare the SEISMIC completion summary")
...
def _register_result_handoff_callback(app: dash.Dash) -> None:
    @app.callback(
        Output("q-location", "href", allow_duplicate=True),
    ...
    def navigate_after_result_handoff(
        ...
        if not _is_seismic_result_handoff(
...
def _register_activity_callback(app: dash.Dash) -> None:
    @app.callback(
        Output(SeismicIds.ACTIVITY_OUTPUT, "children"),
        Input(SeismicIds.ACTIVITY_TABS, "value"),
        Input(SeismicIds.EVENTS, "data"),
    )
    def render_activity(
        selected: str | None,
...
def _register_download_callbacks(
    ...
    @app.callback(
        Output(SeismicIds.DOWNLOAD_HDF5_PAYLOAD, "data"),
        Input(SeismicIds.DOWNLOAD_HDF5, "n_clicks", allow_optional=True),
        State(SeismicIds.SESSION, "data"),
        prevent_initial_call=True,
    )
    ...
    @app.callback(
        Output(SeismicIds.DOWNLOAD_REPORT_PAYLOAD, "data"),
        Input(SeismicIds.DOWNLOAD_REPORT, "n_clicks", allow_optional=True),
        State(SeismicIds.SESSION, "data"),
        prevent_initial_call=True,
    )
    ...
    @app.callback(
        Output(SeismicIds.DOWNLOAD_CSV_PAYLOAD, "data"),
        Input(SeismicIds.DOWNLOAD_CSV, "n_clicks", allow_optional=True),
        State(SeismicIds.SESSION, "data"),
        prevent_initial_call=True,
    )
    def download_csv(clicks: int | None, session: Mapping[str, Any] | None) -> Any:
        active = _active_from_session(clicks, session)
        if active is None:
            return no_update
        path = service.sampled_csv(active)
        return dcc.send_file(path, filename=path.name)

    @app.callback(
        Output(SeismicIds.DOWNLOAD_DIAGNOSTIC_PAYLOAD, "data"),
        Input(SeismicIds.DOWNLOAD_DIAGNOSTIC, "n_clicks", allow_optional=True),
        State(SeismicIds.SESSION, "data"),
        State(SeismicIds.EVENTS, "data"),
        prevent_initial_call=True,
    )
    def download_diagnostic(
        ...
        return dcc.send_string(
            ...
            filename="seismic-diagnostic.txt",
        )
...
def _is_seismic_result_handoff(
    *,
    result_session: Mapping[str, Any] | None,
    workflow_session: Mapping[str, Any] | None,
    pathname: str | None,
) -> bool:
    if dash.strip_relative_path(pathname) != "seismic":
        return False
...
__all__ = ["register_seismic_callbacks"]
