...
def normalize_table(table: Any) -> NormalizedTable:
    ...
    display_rows = tuple(
        tuple(
            format_cell(value, formats[index])
            for index, value in enumerate(_padded(row, len(columns)))
        )
        for row in rows
    )
...
def table_component(
    ...
    *,
    ...
    normalized = normalize_table(table)
    records = [
        {f"c{index}": value for index, value in enumerate(row)} for row in normalized.display_rows
    ]
